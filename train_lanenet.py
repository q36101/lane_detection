...
# from lanenet_model import lanenet_merge_model_resa as lanenet_merge_model
from lanenet_model import  lanenet_merge_model
from data_provider import lanenet_data_processor
# from encoder_decoder_model import mobilenet_v3_large_encoder
import numpy as np
...
def train_net(net_flag, dataset_dir, weights_path=None):
    ...
    # input_tensor = input_tensor_o

    # input = compute_ret['input_tensor']
    # binary_label = compute_ret['binary_label']    #二值分割loss
    # instance_label = compute_ret['instance_label']
    ...
    # atgen_tensor1= compute_ret['input_tensor1'],
    # atgen_tensor2= compute_ret['input_tensor2'],
    # atgen_tensor3= compute_ret['input_tensor3'],
    # atgen_tensor4= compute_ret['input_tensor4'],
 
    # atgen_6 = compute_ret['output_6']
    # atgen_7 = compute_ret['output_7']
    # atgen_t = compute_ret['output_t']
    # atgen_d = compute_ret['output_d']
    # atgen_l = compute_ret['output_l']
    # atgen_r = compute_ret['output_r']
    # atgen_st = compute_ret['output_st']
    # atgen_sd = compute_ret['output_sd']
    # atgen_re = compute_ret['output_re']
    # atgen_re2 = compute_ret['output_re2']
    # atgen_r1 = compute_ret['result_1']
    # atgen_d1 = compute_ret['deconv_1']
...
#限制GPU使用    sess_config.gpu_options.allow_growth = True
    ...
    sess_config.gpu_options.allow_growth = CFG.TRAIN.TF_ALLOW_GROWTH
    sess_config.gpu_options.allocator_type = 'BFC'
    # run_config = tf.estimator.RunConfig().replace(sess_config=sess_config)
    ...
    with sess.as_default():
        ...
        for epoch in range(train_epochs):
            ...
            #     # print('binary_gt_labels',binary_gt_labels)
            #     # print('instance_gt_labels',instance_gt_labels)
            ...
            #     image = np.concatenate((gt_imgs, binary_gt_labels, instance_gt_labels), axis=-1)     #有1/2的機率會做水平翻轉來增加data數量
            ...
            #     #判斷是否變色
            #     if random.random() < 0.5:
            #         gt_imgs = [cv2.applyColorMap(tmp, color_map[random.randint(0,20)]) for tmp in gt_imgs]                              
            #     else:
            #         gt_imgs=gt_imgs
            #     #調整大小
            ...
            phase_train = 'train'
            '''output_6,output_t,output_d,output_l,output_r,output_st,output_sd,output_re,output_re2,output_d1,output_r1,'''
            _, c, train_accuracy, train_summary, binary_loss, instance_loss, l2, dice, focal, embedding, binary_seg_img,output_1,output_2,output_3,output_4,output_5,= \
            ...
            # print('111111111111111111111111111111111111111111111111',sess.run)
            # print('cccccccccccccccccccccccccccccccccccccccccccccccc',c)
            # print('cccccccccccccccccccccccccccccccccccccccccccccccc', instance_loss)
            ...
            if math.isnan(c) or math.isnan(binary_loss) or math.isnan(instance_loss):      #若c、binary_loss、instance_loss為nan(出問題ㄌ)則跑這個if下面的
                ...
                return
            if epoch % 100 == 0:
                cv2.imwrite('./overseed/gt_image.png', gt_imgs[1] + VGG_MEAN)
                # cv2.imwrite('./overseed/input_image.png', input_image[1] + VGG_MEAN)

                cv2.imwrite('./overseed/binary_label.png', binary_gt_labels[1] * 255)
                # cv2.imwrite('./overseed/input_binary_label.png',input_binary_label[1] * 255)

                cv2.imwrite('./overseed/instance_label.png', instance_gt_labels[1])
                # cv2.imwrite('./overseed/input_instance_label.png', input_instance_label[1])

                cv2.imwrite('./overseed/binary_seg_img.png', binary_seg_img[1] * 255)
                # cv2.imwrite('./overseed/embedding.png', embedding[1])

                cv2.imwrite('./overseed/embedding_color.png', embedding[0]*100)
                # cv2.imwrite('./overseed/output_0.png', output_0[1]*100)
                cv2.imwrite('./overseed/output_1.png', output_1[1]*100)
                cv2.imwrite('./overseed/output_2.png', output_2[1]*100)
                cv2.imwrite('./overseed/output_3.png', output_3[1]*100)
                cv2.imwrite('./overseed/output_4.png', output_4[1]*100)
                cv2.imwrite('./overseed/output_5.png', output_5[1]*100)
                # cv2.imwrite('./overseed/atgen_tensor1.png', atgen_tensor1)
                # cv2.imwrite('./overseed/atgen_tensor2.png', atgen_tensor2)
                # cv2.imwrite('./overseed/atgen_tensor3.png', atgen_tensor3)
                # cv2.imwrite('./overseed/atgen_tensor4.png', atgen_tensor4)
                # cv2.imwrite('./overseed/output_6.png', output_6[1]*200)
                # cv2.imwrite('./overseed/output_7.png', output_7[1]*200)
                # cv2.imwrite('./overseed/output_t.png', output_t[1]*255)
                # cv2.imwrite('./overseed/output_d.png', output_d[1]*255)
                # cv2.imwrite('./overseed/output_l.png', output_l[1]*255)
                # cv2.imwrite('./overseed/output_r.png', output_r[1]*255)
                # cv2.imwrite('./overseed/output_st.png', output_st[1]*255)
                # cv2.imwrite('./overseed/output_sd.png', output_sd[1]*255)
                # cv2.imwrite('./overseed/output_re.png', output_re[1]*255)
                # cv2.imwrite('./overseed/output_re2.png', output_re2[1]*255)

                # cv2.imwrite('./overseed/output_d1.png', output_d1[1]*100)
                # cv2.imwrite('./overseed/output_r1.png', output_r1[1]*100)
            if epoch % 1000 == 0:
                cv2.imwrite('./overseed/see/image '+str(count)+'.png', gt_imgs[0] + VGG_MEAN)
                # cv2.imwrite('./overseed/see/input_image '+str(count)+'.png', input_image[0] + VGG_MEAN)

                cv2.imwrite('./overseed/see/binary_label '+str(count)+'.png', binary_gt_labels[0] * 255)
                # cv2.imwrite('./overseed/see/input_binary_label '+str(count)+'.png',input_binary_label[0] * 255)

                cv2.imwrite('./overseed/see/instance_label '+str(count)+'.png', instance_gt_labels[0])
                # cv2.imwrite('./overseed/see/input_instance_label '+str(count)+'.png', input_instance_label[0])
                ...
                # cv2.imwrite('./overseed/see/output_0 '+str(count)+'.png', output_0[0]*100)
                ...
                # cv2.imwrite('./overseed/see/output_6 '+str(count)+'.png', output_6[0]*200)
                # # cv2.imwrite('./overseed/see/output_7 '+str(count)+'.png', output_7[0]*200)
