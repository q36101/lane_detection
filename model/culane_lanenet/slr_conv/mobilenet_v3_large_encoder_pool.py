        ...
        with tf.compat.v1.variable_scope(name):
            ...
            for cnt in range(3, conv8_1.get_shape().as_list()[1]): 
                with tf.compat.v1.variable_scope("convs_10_1", reuse=True):
                    ...
                    print('')
                    print('feature_list_new',feature_list_new)
                    conv_10_1=tf.add(feature_list_old[cnt-1],feature_list_old[cnt-2])
                    conv_10_1_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_1, 320, (1, 9), strides=(1, 1), padding='SAME'))
                    conv_10_1=tf.add(feature_list_old[cnt],conv_10_1_add)
                    feature_list_new.append(conv_10_1_add)
            ...
            with tf.compat.v1.variable_scope("convs_10_2"):
                ...
                conv_10_2 = feature_list_old[length]
                conv_10_2_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_1, 320, (1, 9), strides=(1, 1), padding='SAME'))
                conv_10_2_add = tf.add(conv_10_2_add,feature_list_old[length-1])
                ...
                conv_10_2_add = tf.add(conv_10_2,feature_list_old[length-2])
                feature_list_new.append(conv_10_2_add)
            ...
            for cnt in range(3, conv8_1.get_shape().as_list()[1]):  #把剩下的片全部照做
                with tf.compat.v1.variable_scope("convs_10_2", reuse=True):
                    ...
                    # conv_10_2_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_2, 320, (1, 9), strides=(1, 1), padding='SAME'))
                    # feature_list_new.append(conv_10_2_add)
                    conv_10_2=tf.add(feature_list_old[length-(cnt-1)],feature_list_old[length-(cnt-2)])
                    conv_10_2_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_2, 320, (1, 9), strides=(1, 1), padding='SAME'))
                    conv_10_2=tf.add(feature_list_old[length-cnt],conv_10_2_add)
                    feature_list_new.append(conv_10_2_add)
            ...
            for cnt in range(3, conv8_1.get_shape().as_list()[2]):
                with tf.compat.v1.variable_scope("convs_10_3", reuse=True):
                    print('cnt=',cnt,"+",cnt-1,"+",cnt-2)
                    print('')
                    conv_10_3=tf.add(feature_list_old[cnt-1],feature_list_old[cnt-2])
                    conv_10_3_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_3, 320, (1, 9), strides=(1, 1), padding='SAME'))
                    conv_10_3=tf.add(feature_list_old[cnt],conv_10_3_add)
                    feature_list_new.append(conv_10_3_add)
            ...
            for cnt in range(3, conv8_1.get_shape().as_list()[2]):
                with tf.compat.v1.variable_scope("convs_10_4", reuse=True):
                    print('cnt=',length-cnt,"+",length-(cnt-1),"+",length-(cnt-2))
                    print('')
                    conv_10_4=tf.add(feature_list_old[length-(cnt-1)],feature_list_old[length-(cnt-2)])
                    conv_10_4_add = tf.nn.relu(tf.compat.v1.layers.separable_conv2d(conv_10_4, 320, (1, 9), strides=(1, 1), padding='SAME'))
                    conv_10_4=tf.add(feature_list_old[length-cnt],conv_10_4_add)
                    feature_list_new.append(conv_10_4_add)
            ...
            inner_product_top = tf.math.add(reshape_top, reshape_down)

            softmax_top = tf.nn.softmax(tf.matmul(reshape_hidden, inner_product_top))
            
            inner_product_top = tf.transpose(inner_product_top, [1, 0])
            h_weighted = tf.matmul(softmax_top,inner_product_top)
        
            
            inner_product_down = tf.math.add(reshape_right, reshape_left)
            
            softmax_down = tf.nn.softmax(tf.matmul(reshape_hidden, inner_product_down))
            inner_product_down = tf.transpose(inner_product_down, [1, 0])
            
            w_weighted = tf.matmul(softmax_down, inner_product_down)
            
            weighted_product = tf.math.add(h_weighted, w_weighted)
            
            feature_sum = tf.math.add(reshape_hidden, weighted_product)
