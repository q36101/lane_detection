...
        :return: 輸出MoblieNet_v3編碼特徵
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            # conv1_1 = Dropout(conv1_1)

            # conv1_1=PatchEmbed(patch_size=(16,16), stride=(16,16), padding=0, in_chans=3, embed_dim=768)(conv1_1)
            ###POOL_FORMER-1####
            x = conv1_1
            N, H, W, C = x.shape
            y = conv1_1[:,0:int(H/4),:,:]
            ...
            x = rearrange(x ,'b (h p1) (w p2) c -> b h w  (p1 p2 c)',p1=int(kernel_size),p2=int(kernel_size))
