...
#import mobilenet_v3_large_encoder_resa
#from encoder_decoder_model import mobilenet_v3_large_encoder_resa
from encoder_decoder_model import mobilenet_v3_large_encoder
from encoder_decoder_model import mobilenet_v3_large_decoder
...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    實現語義分割模型
    """
    def __init__(self, phase, net_flag='mv3'): #net_flag要記得改!!!!!!!!!!!!!!!!!!!!!!! 指定vgg只是方便run這個py檔
        """
        ...
        if self._net_flag == 'vgg':
            ...
            self._encoder = mobilenet_v3_large_encoder.mobilenet_v3_large_Encoder(phase=phase)
            self._decoder = mobilenet_v3_large_decoder.mobilenet_v3_large_Decoder()
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            if self._net_flag.lower() == 'vgg':
                ...
                decode_ret = self._decoder.decode(input_tensor_dict=encode_ret,
                                                  ...
                                                  fpn_concat_list=['bneck6_2', 'bneck4_3', 'bneck3_2'],    #拿掉最前面'bneck6_2'，加'bneck2_1'
                                                  attention_map_list=['output_5','output_4',
                                                                      'output_3','output_2','output_1',
                                                                    #   ,'output_6','output_t','output_d','output_l','output_r','output_st','output_sd','output_re','output_re2',
                                                                    #   ,'output_td','output_lr'
                                                                    ])
                return decode_ret 
    ...
    def compute_loss(self, input_tensor, binary_label, instance_label ,name):
        ...
        """
        # if flip:
        #     input_tensor = tf.image.flip_left_right(input_tensor)
        #     binary_label = tf.image.flip_left_right(binary_label)
        #     instance_label = tf.image.flip_left_right(instance_label)
        # else:
...
#            sad_loss = tf.add(tf.reduce_sum((inference_ret['output_4']-inference_ret['output_3'])**2)/2, sad_loss)
            ...
            binary_label = tf.squeeze(binary_label, squeeze_dims=[3])           #把binary_label的第三個值去掉
            ...
            #     name='entropy_loss')
            binary_segmenatation_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
...
#TEST            binary_segmenatation_loss = tf.nn.l2_loss(decode_logits_reshape)
            ...
            disc_loss, l_var, l_dist, l_reg = \
                lanenet_discriminative_loss.discriminative_loss(
                    pix_embedding, instance_label, 3, image_shape, 0.5, 3, 1.0, 1.0, 0.001)        #算實例分割的loss(disc_loss是裡面的total_loss) !!!!!!!1.5可以改3試試!!
                
            #Dice loss
            TP = tf.count_nonzero(tf.multiply(tf.cast(tf.equal(binary_label_for_dice, 1), tf.int32),     #將預測的圖和gt做相乘，算交集(PIXEL值為1)的點的個數
            ...
            union_1 = tf.add(tf.count_nonzero(tf.cast(tf.equal(binary_label_for_dice, 1),        #將預測為線的PIXEL數和GT為線的PIXEL數相加，算聯集
                                              tf.int32), dtype=tf.int32),
                             tf.count_nonzero(tf.cast(tf.equal(decode_logits, 1),
                                              tf.int32), dtype=tf.int32))
            ...
            # 合併損失
            total_loss = binary_segmenatation_loss + dice_loss + 0.001 * l2_loss + focal_loss #+ 0.1 * sad_loss  # 0.3 * disc_loss
...
#               'sad_loss': sad_loss,
                ...
                # 'output_7': inference_ret['output_7'],
                # 'output_t': inference_ret['output_t'],
                # 'output_d': inference_ret['output_d'],
                # 'output_l': inference_ret['output_l'],
                # 'output_r': inference_ret['output_r'],
                # 'output_st': inference_ret['output_st'],
                # 'output_sd': inference_ret['output_sd'],
                # 'output_re': inference_ret['output_re'],
                # 'output_re2': inference_ret['output_re2'],
                # 'deconv_1': inference_ret['deconv_1'],
                # 'result_1': inference_ret['result_1'],
            #    'output_0': inference_ret['top'],
    ...
    def inference(self, input_tensor, name):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            # decode_instance = inference_ret['instance']
            # 像素嵌入
            pix_embedding1 = self.conv2d(inputdata=decode_deconv, out_channel=3, kernel_size=1,
                                        use_bias=False, name='pix_embedding_conv')

            pix_embedding1 = self.relu(inputdata=pix_embedding1, name='pix_embedding_relu')       # = (1, 256, 512, 3)

            # #多分類測試=5類
            # decode_instance = inference_ret['instance']
            # pix_embedding2 = self.conv2d(inputdata=decode_instance, out_channel=3, kernel_size=1,
            #                             use_bias=False, name='pix_embedding_conv2')
            # pix_embedding2 = self.relu(inputdata=decode_instance, name='pix_embedding_relu2')

            # #多分類測試=5類+法二
            # decode_deconv = inference_ret['deconv']
            # pred= decode_deconv[0]
            # colormap=[[255,255,255],[0,255,0],[0,0,255],[255,0,0],[255,255,0]]
            # cm=np.array(colormap).astype(np.uint8)
            # pred = np.argmax(np.array(pred), axis=-1)
            # pred_val=cm[pred]
            # pred.squeeze()

            return binary_seg_ret ,pix_embedding1
...
if __name__ == '__main__':
    ...
    IoU= model.compute_loss(input_tensor=input_tensor, binary_label=binary_label,
