...
from lanenet_model import lanenet_merge_model_trans
from lanenet_model import lanenet_cluster
from lanenet_model import lanenet_postprocess
from config import global_config
# import image_detesize
...
def test_lanenet(image_path, weights_path, net_flag, use_gpu):
    ...
    net = lanenet_merge_model_trans.LaneNet(phase=phase_tensor, net_flag=net_flag)
...
#        with sess.as_default():
            ...
            for epoch in range(epoch_nums):
                ...
                for index, binary_seg_image in enumerate(binary_seg_images):
                    ...
                    #images = binary_seg_image*(instance_seg_images/255)
                    ...
                    test = cv2.resize(binary_seg_image, (image_vis_list[index].shape[1],           #這邊的test為黑背景且線是白色的(binary)(720*1280原圖大小)
                                                         image_vis_list[index].shape[0]),
                                            interpolation=cv2.INTER_LINEAR)
                    # test = cv2.resize(binary_seg_image, (1280,           #這邊的test為黑背景且線是白色的(binary)(720*1280原圖大小)
                    #                                      720),
                    #                         interpolation=cv2.INTER_LINEAR)
                    ...
                    #     # cv2.imwrite(image_save_path,  test2*100)
                    #     # cv2.imwrite(image_save_path, ans*-10)
