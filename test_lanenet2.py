...
def test_lanenet_batch(image_dir, weights_path, net_flag, use_gpu, save_dir=None ,with_lane_fit=True):
    """
...
#        with sess.as_default():
            ...
            for epoch in range(epoch_nums):
                ...
                ############################
                # i=epoch
                # red_image1=binary_seg_images
                # red_image1=np.transpose(binary_seg_images,[0,1,2])
                # red_image1=np.tile(binary_seg_images[0], [1, 1, 3])
                # red_image1=np.transpose(np.tile(binary_seg_images[0], [1, 1, 3]), [2, 0, 1])
                # red_image1[0] = red_image1[0] * 255
                # red_image1[1] = red_image1[1] * 0
                # red_image1[2] = red_image1[2] * 0
                # red_image1 = np.transpose(red_image1, [1, 2, 0])
                # overlay1 = (red_image1 * 0.4 + i * 0.6).astype(np.uint8)
                # print('1111111111111111111',overlay1,type(overlay1),overlay1.shape)
                # cv2.imwrite(cv2.cvtColor(overlay1, cv2.COLOR_RGB2BGR))
                
                # red_image2=np.transpose(np.tile(instance_seg_images[0], [1, 1, 3]), [2, 0, 1])
                # red_image2[0] = red_image2[0] * 255
                # red_image2[1] = red_image2[1] * 0
                # red_image2[2] = red_image2[2] * 0
                # red_image2 = np.transpose(red_image2, [1, 2, 0])
                # overlay2 = (red_image2 * 0.4 + i * 0.6).astype(np.uint8)


                #塗色
                
                
                # red_image1=red_image1[0]*255
                # # print('1111111111111111111',red_image1,type(red_image1),red_image1.shape)
                # color_dict = {"black": [0, 0, 0], 
                #                 "red": [200, 0, 0], 
                #                 "pink": [255, 192, 203]}
                # rgb_image = np.zeros(shape=(*red_image1.shape, 3))
                # for i in range(rgb_image.shape[0]):
                #     for j in range(rgb_image.shape[1]):
                #         # 3：对不同的灰度值选择不同的颜色
                #         if red_image1[i, j] > 15:
                #             rgb_image[i, j, :] = color_dict["red"]
                # print('1111111111111111111',red_image1,type(red_image1),red_image1.shape)
                ############################
                # print('1111111111111111111',decode_image,type(decode_image),decode_image.shape)
                cluster_time = []
                for index, binary_seg_image in enumerate(binary_seg_images):
                    ...
                    # red_image1=binary_seg_image*255
                    ...
                    #         if red_image1[i, j] > 15:
                    #             rgb_image[i, j, :] = color_dict["red"]
                    # rgb_image = cv2.resize(rgb_image, (image_vis_list[index].shape[1],           
                    ...
                    # np.array(lane_embedding_feats, np.float32)
                    # np.array(lane_coordinate, np.int64)
                    
                    # prediction=lane_embedding_feats
                    # # ms = MeanShift(bandwidth=1.5, bin_seeding=True)
                    # ms = MeanShift(args['bandwidth'], bin_seeding=True)
                    # labels = ms.fit(prediction)
                    # try:
                    ...
                    test2 = cv2.resize(instance_seg_images[0], (image_vis_list[index].shape[1],           
                                                         image_vis_list[index].shape[0]),
                                            interpolation=cv2.INTER_LINEAR)
                    print('111111111111111',type(test),type(test2))
                    # test=np.dot(test,test2)
                    test=cv2.cvtColor(test,cv2.COLOR_GRAY2BGR)

                    # test2=test2*test2
                    test=test*mask_image
                    #print('1111111111111111111',test2,type(test2),test2.shape)
            ...
            print('進行車道線聚類的部分平均每張耗時:', time_cluster/(epoch_nums*CFG.TRAIN.T)+1)
