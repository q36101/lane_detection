...
class CNNBaseModel(object):
    ...
    def conv2d(inputdata, out_channel, kernel_size, padding='SAME',
        ...
        with tf.compat.v1.variable_scope(name):
            in_shape = inputdata.get_shape().as_list()#get_shape()，只有tensor才可以使用这种方法，返回的是一个元组，需要通过as_list()的操作转换成list.
            channel_axis = 3 if data_format == 'NHWC' else 1
            in_channel = in_shape[channel_axis]
            assert in_channel is not None, "[Conv2D] Input cannot have unknown channel"
            assert in_channel % split == 0 # assert:表示必定滿足該條件
            assert out_channel % split == 0

            padding = padding.upper()#"小写字母转为大写字母的字符串

            if isinstance(kernel_size, list):# if true:如果kernel_size的參數類型和list相同
    ...
    def maxpooling(inputdata, kernel_size, stride=None, padding='VALID',
        ...
        :return:
        """
        padding = padding.upper()#返回小写字母转为大写字母的字符串
        ...
        return tf.compat.v1.nn.max_pool(value=inputdata, ksize=kernel, strides=strides, padding=padding,
    ...
    def layerbn(inputdata, is_training, name):
        ...
        def f1():
            ...
            return tf_layer.LayerNormalization(
...
if __name__ == '__main__':
    ...
    print("2",strides)
    print("3",inputs,outputs,kernels,conv)