...
class mobilenet_v3_large_Encoder(cnn_basenet.CNNBaseModel):
    """
    ...
    def conv2d(self, input_, output_dim, k_h, k_w, d_h, d_w, stddev=0.09, name='conv2d', bias=False):
        with tf.variable_scope(name):
            ...
            return conv
    def atrous_conv2d(self, input_, output_dim, k_h, k_w, d_h, d_w, rate,stddev=0.09, name='conv2d', bias=False):
        with tf.variable_scope(name):
            w = tf.get_variable('w', [k_h, k_w, input_.get_shape()[-1], output_dim],
                  regularizer=tf.contrib.layers.l2_regularizer(weight_decay),
                  initializer=tf.truncated_normal_initializer(stddev=stddev))
    ...
    def conv2d_block(self, input, out_dim, k, s, is_train, name, h_swish=False):
        with tf.name_scope(name), tf.variable_scope(name):
            ...
            if h_swish == True:
                ...
                net = self.relu6(net)
            return net
    def atrous_conv2d_block(self, input, out_dim, k, s, rate,is_train, name, h_swish=False):
        with tf.name_scope(name), tf.variable_scope(name):
            net = self.atrous_conv2d(input, out_dim, k, k, s, s, rate,name='conv2d')
            net = self.batch_norm(net, train=is_train, name='bn')
            if h_swish == True:
                net = self.hard_swish(net)
            else:
...
        :return: 輸出MoblieNet_v3編碼特徵
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            ret['input_tensor'] = dict() # 字典
            ret['input_tensor']['data'] = input_tensor
            ret['input_tensor']['shape'] = input_tensor.get_shape().as_list()
            # conv stage 1
            ...
            ##############atrous_conv2d_block - 1################
            atrous_conv2d_1 = self.atrous_conv2d_block(conv1_1, 16, 3, 2, rate=2 , is_train=True, name='conv1_2', h_swish=True)  #             
            ret['atrous_conv2d_1'] = dict() # 字典
            ret['atrous_conv2d_1']['data'] = atrous_conv2d_1
            ret['atrous_conv2d_1']['shape'] = atrous_conv2d_1.get_shape().as_list()  #[2, 128, 256, 16]

            atrous_conv2d_2 = self.atrous_conv2d_block(atrous_conv2d_1, 16, 3, 2, rate=4 , is_train=True, name='conv1_3', h_swish=True)  #          
            ret['atrous_conv2d_2'] = dict() # 字典
            ret['atrous_conv2d_2']['data'] = atrous_conv2d_2
            ret['atrous_conv2d_2']['shape'] = atrous_conv2d_2.get_shape().as_list()  #[2, 128, 256, 16]

            ##############atrous_conv2d_block - 1################

            # conv stage 2
            bneck2_1 = self.mnv3_block(atrous_conv2d_2, 3, 16, 16, 1, is_train=True, name='bneck2_1', h_swish=False, ratio=reduction_ratio, se=False)
            ...
            ##############atrous_conv2d_block - 2################
            atrous_conv2d_3 = self.atrous_conv2d_block(bneck3_2, 16, 3, 2, rate=4 , is_train=True, name='atrous_conv2d_3', h_swish=True)  #             
            ret['atrous_conv2d_3'] = dict() # 字典
            ret['atrous_conv2d_3']['data'] = atrous_conv2d_3
            ret['atrous_conv2d_3']['shape'] = atrous_conv2d_3.get_shape().as_list()  #[2, 64, 128, 24]


            atrous_conv2d_4 = self.atrous_conv2d_block(atrous_conv2d_3, 16, 3, 2, rate=8 , is_train=True, name='atrous_conv2d_4', h_swish=True)  # 
            ret['atrous_conv2d_4'] = dict() # 字典
            ret['atrous_conv2d_4']['data'] = atrous_conv2d_4
            ret['atrous_conv2d_4']['shape'] = atrous_conv2d_4.get_shape().as_list()  #[2, 64, 128, 24]


            ##############atrous_conv2d_block - 2################
            
            output_2 = tf.image.resize(tf.expand_dims(tf.reduce_sum(tf.abs(atrous_conv2d_4)**2, axis=3), axis=3), (128, 256)) 
            ...
            # conv stage 4
            bneck4_1 = self.mnv3_block(atrous_conv2d_4, 5, 72, 40, 2, is_train=True, name='bneck4_1', h_swish=False, ratio=reduction_ratio, se=False)  # size/8
            bneck4_2 = self.mnv3_block(bneck4_1, 5, 120, 40, 1, is_train=True, name='bneck4_2', h_swish=False, ratio=reduction_ratio, se=False)
            bneck4_3 = self.mnv3_block(bneck4_2, 5, 120, 40, 1, is_train=True, name='bneck4_3', h_swish=False, ratio=reduction_ratio, se=False)
            
            ret['bneck4_3'] = dict()
            ret['bneck4_3']['data'] = bneck4_3
            ret['bneck4_3']['shape'] = bneck4_3.get_shape().as_list()	#fpn_concat_2 [2, 32, 64, 40]

            ##############atrous_conv2d_block - 3 ################

            atrous_conv2d_5 = self.atrous_conv2d_block(bneck4_3, 16, 3, 2, rate=4 , is_train=True, name='atrous_conv2d_5', h_swish=True)  #             
            ret['atrous_conv2d_5'] = dict() # 字典
            ret['atrous_conv2d_5']['data'] = atrous_conv2d_5
            ret['atrous_conv2d_5']['shape'] = atrous_conv2d_5.get_shape().as_list()  #[2, 32, 64, 40]


            atrous_conv2d_6 = self.atrous_conv2d_block(atrous_conv2d_5, 16, 3, 2, rate=4 , is_train=True, name='atrous_conv2d_6', h_swish=True)  #    
            ret['atrous_conv2d_6'] = dict() # 字典
            ret['atrous_conv2d_6']['data'] = atrous_conv2d_6
            ret['atrous_conv2d_6']['shape'] = atrous_conv2d_6.get_shape().as_list()  #[2, 32, 64, 40]

            ##############atrous_conv2d_block - 3 ################
            
            output_3 = tf.image.resize(tf.expand_dims(tf.reduce_sum(tf.abs(atrous_conv2d_6)**2, axis=3), axis=3), (128, 256))
            ...
            ##############atrous_conv2d_block - 4 ################

            atrous_conv2d_7 = self.atrous_conv2d_block(bneck5_4, 16, 3, 2, rate=2 , is_train=True, name='atrous_conv2d_7', h_swish=True)  #             
            ret['atrous_conv2d_7'] = dict() # 字典
            ret['atrous_conv2d_7']['data'] = atrous_conv2d_7
            ret['atrous_conv2d_7']['shape'] = atrous_conv2d_7.get_shape().as_list()  #[2, 32, 64, 40]


            atrous_conv2d_8 = self.atrous_conv2d_block(atrous_conv2d_7, 16, 3, 2, rate=1 , is_train=True, name='atrous_conv2d_8', h_swish=True)  #    
            ret['atrous_conv2d_8'] = dict() # 字典
            ret['atrous_conv2d_8']['data'] = atrous_conv2d_8
            ret['atrous_conv2d_8']['shape'] = atrous_conv2d_8.get_shape().as_list()  #[2, 32, 64, 40]

            ##############atrous_conv2d_block - 4 ################

            # conv_stage 6
            bneck6_1 = self.mnv3_block(atrous_conv2d_8, 3, 480, 112, 1, is_train=True, name='bneck6_1', h_swish=True, ratio=reduction_ratio, se=False)
            bneck6_2 = self.mnv3_block(bneck6_1, 3, 672, 112, 1, is_train=True, name='bneck6_2', h_swish=True, ratio=reduction_ratio, se=False)
            ...
            # # conv stage 7
            # bneck7_1 = self.mnv3_block(bneck6_2, 5, 672, 160, 2, is_train=True, name='bneck7_1', h_swish=True, ratio=reduction_ratio, se=False) # size/32
            # bneck7_2 = self.mnv3_block(bneck7_1, 5, 960, 160, 1, is_train=True, name='bneck7_2', h_swish=True, ratio=reduction_ratio, se=False)
            # bneck7_3 = self.mnv3_block(bneck7_2, 5, 960, 160, 1, is_train=True, name='bneck7_3', h_swish=True, ratio=reduction_ratio, se=False) 
            ...
            # reshape_hidden = tf.reshape(conv8_1, (conv8_1.shape[3].value, (conv8_1.shape[0].value)*(conv8_1.shape[1].value)*(conv8_1.shape[2].value)))
            
            #     ################################################################### top to down ###################################################################
            # feature_list_old = []
            # feature_list_new = []
            # for cnt in range(conv8_1.get_shape().as_list()[1]):  #看feature map的第二維(高256那維)是多少就跑多少次
            #     feature_list_new.append(tf.expand_dims(conv8_1[:, cnt, :, :], axis=1))  #把每一片feature map存到feature_list_old
            # # feature_list_new.append(tf.expand_dims(conv8_1[:, 0, :, :], axis=1))  #將第一片存到feature_list_new
            
            # # with tf.compat.v1.variable_scope("convs_10_1"):
            # #     conv_10_1 = tf.add(tf.nn.relu(tf.layers.separable_conv2d(feature_list_old[0], 320, (1, 13), strides=(1, 1), padding='SAME')),
            # #                       feature_list_old[1])  #先對第一片做conv，再將做完的結果和原始的第二片相加
            # #     feature_list_new.append(conv_10_1)  #將相加完的結果存到feature_list_new(原本裡面有一個原始第一片)

            # # for cnt in range(2, conv8_1.get_shape().as_list()[1]):  #把剩下的片全部照做
            # #     with tf.compat.v1.variable_scope("convs_10_1", reuse=True):
            # #         conv_10_1 = tf.add(tf.nn.relu(tf.layers.separable_conv2d(feature_list_new[cnt - 1], 320, (1, 13), strides=(1, 1), padding='SAME')),
            ...
            # reshape_top = tf.reshape(processed_feature_top, ((processed_feature_top.shape[0].value)*(processed_feature_top.shape[1].value)*(processed_feature_top.shape[2].value), processed_feature_top.shape[3].value))
            
            #     ################################################################### down to top ###################################################################
            # # feature_list_old = feature_list_new
            # # feature_list_new = []
            # # length = int(CFG.TRAIN.IMG_HEIGHT / 32) - 1  #32是feature map的高縮小的倍數，這裡從256縮小變8
            # # feature_list_new.append(feature_list_old[length])  #將top to down做完的結果的最下面(最後做完)的那片存到feature_list_new當作第一片
            
            # # with tf.compat.v1.variable_scope("convs_10_2"):
            # #     conv_10_2 = tf.add(tf.nn.relu(tf.layers.separable_conv2d(feature_list_old[length], 320, (1, 13), strides=(1, 1), padding='SAME')),
            # #                       feature_list_old[length - 1])  #對top to down做完的feature map從底部做conv，再將做完的結果和底部的上一片相加
            # #     feature_list_new.append(conv_10_2)  #將相加完的結果存到feature_list_new(原本裡面有一個top to down做完後的最後一片)

            # # for cnt in range(2, conv8_1.get_shape().as_list()[1]):  #把剩下的片全部照做
            # #     with tf.compat.v1.variable_scope("convs_10_2", reuse=True):
            # #         conv_10_2 = tf.add(tf.nn.relu(tf.layers.separable_conv2d(feature_list_new[cnt - 1], 320, (1, 13), strides=(1, 1), padding='SAME')),
            ...
            # reshape_down = tf.reshape(processed_feature_dowm, ((processed_feature_dowm.shape[0].value)*(processed_feature_dowm.shape[1].value)*(processed_feature_dowm.shape[2].value), processed_feature_dowm.shape[3].value))
            
            #     ################################################################### left to right ###################################################################
            # feature_list_old = []
            # feature_list_new = []
            # for cnt in range(conv8_1.get_shape().as_list()[2]):
            #     feature_list_new.append(tf.expand_dims(conv8_1[:, :, cnt, :], axis=2))
            ...
            # reshape_left = tf.reshape(processed_feature_left, ((processed_feature_left.shape[0].value)*(processed_feature_left.shape[1].value)*(processed_feature_left.shape[2].value), processed_feature_left.shape[3].value))
            ...
            # reshape_right = tf.reshape(processed_feature_right, ((processed_feature_right.shape[0].value)*(processed_feature_right.shape[1].value)*(processed_feature_right.shape[2].value), processed_feature_right.shape[3].value))
            ...
            # result = tf.reshape(feature_sum, (conv8_1.shape[0].value, conv8_1.shape[1].value, conv8_1.shape[2].value, conv8_1.shape[3].value))
            # result=conv8_1
            result=bneck6_2
